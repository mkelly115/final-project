...
const MyOverview = () => {
    ...
    const {
      ...
      const { loading: projectsLoading, error: projectsError, data: projectsData } = useQuery(QUERY_SINGLE_USER, {
    ...
    const projectChartData = Object.entries(projectStatusCounts).map(([status, count], index) => ({
    ...
    const taskChartData = Object.entries(taskStatusCounts).map(([status, count], index) => ({
        id: `status_${status}_${index}`, // Unique ID for each status
        value: count,
        label: status,
    }));
